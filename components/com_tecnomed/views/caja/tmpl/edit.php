...
<script language="javascript" type="text/javascript">
	function submitbutton(task)
        {
                if (task == 'caja.cancel' || document.formvalidator.isValid(document.id('item-form'))) {
                        submitform(task);
                } else {
...
<form action="<?php JRoute::_('index.php?option=com_tecnomed'); ?>" method="post" name="adminForm" id="item-form" class="form-validate">
		...
		<fieldset class="adminform">
			<legend><?php echo JText::_( 'COM_TECNOMED_FIELDSET_DATOS_PAGO' ); ?></legend>
		...
		<fieldset class="adminform">
			<legend><?php echo JText::_( 'COM_TECNOMED_FIELDSET_PAGO' ); ?></legend>
			<ul class="adminformlist">
			<?php foreach($this->form->getFieldset('caja') as $field): ?>
							<li><?php echo $field->label;echo $field->input;?></li>
			<?php endforeach; ?>
			</ul> 
		 </fieldset>
		 ...
		 <input type="hidden" name="option" value="com_tecnomed" />
	      <input type="hidden" name="task" value="caja.save" />
